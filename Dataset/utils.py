...
# # def save_image(img, path, mode):
#     #print(type(img))  # 查看img的类型
#     #print(img.dtype)  # 如果是numpy数组，这将显示数组的数据类型
#     # save_img = torch.from_numpy( )
#     img_tensor = torch.from_numpy(img)
#     #print("img_tensor:", type(img_tensor))

#     save_img = img_tensor.squeeze().clamp(0, 1).numpy().transpose(1,2,0)
    # save img
    #save_dir = os.path.join(path, str(self.now_time))
    # save_dir = os.path.join(self.cfg['test']['save_dir'], self.cfg['test']['type']+ "_" + str(self.cfg['stage']) + 'stage_inn9', str(self.now_time))
    ...
    #         array = array.cpu().numpy()  # 如果在 GPU 上计算，需要先将 tensor 移至 CPU
...
#     #normalized_list = []
#     max_value = 2 ** bit_depth - .5
#     #print("imge_dict:", img_dict)
#     ret = dict()
    # img_res = img_dict / max_value
    ...
    #     #     continue
    #     # imgs = img_dict[img_name]
    #     img_res = img / max_value
    #     normalized_list.append(img_res)
